jQuery(function($) {
    ...
    $("#feedback").on("click", "input", function(event) {
        ...
        $.post("/user/send_feedback", {
            ...
            if (feedback.status == "ok") {
                showMessage({
                    type: "success",
                    msg: "谢谢您的反馈，我们会在24小时之内查看处理。ㄟ(◑‿◐ )ㄏ"
                });
                $("#feedback .content").val("").text("");
            } else {
                showMessage({
                    type: "error",
                    msg: "⊙０⊙，发送反馈失败，暂时将反馈保存到便签里吧，或者邮件发送到eff505@gmail.com，谢谢"
                });
            }
        });
    });
    ...
    function initialize() {
        ...
        $(".note.editable .favicon.unloaded").load_img_onscroll({
            timeout: 5000,
            fallback_src: "layout/images/favicons.png",
            loading_class: "unloaded"
        }, function() {
            ...
            $(document).on(moveEvent, function(event) {
                ...
                if (mousedown) {
                    if (!tag_con_clone) {
                        ...
                        tag_con_clone = $(tag_con).clone(true).addClass("clone").css({
                            position: "absolute",
                            left: tag_left,
                            top: tag_top,
                            opacity: 1,
                    ...
                    var delete_note_timeout = setTimeout(function() {
                        if ($tag.hasClass("to-be-deleted")) {
                            ...
                            tag.del(function(data) {
                                ...
                                //隐藏提示
                                $(".feedback-hint").fadeOut("fast", function() {
                                    $(this).remove();
                                });
                            });
                        } else {
        ...
        $("#search_area").on("click " + downEvent, ".pined-tags a.tag,.custom-tags a.tag", function(event) {
            ...
            if ($("#search_area .by-tag").hasClass("editing")) {
                ...
                if ($(target).hasClass("close")) {
                    ...
                    if ($tag.parent().hasClass("setting-color")) {
                        ...
                        $("#colors .triangle,#colors .triangle-border").css({
                            left: triangle_left
                        });
                    }
                }
            ...
            $("#search_area .by-tag .tag[data-id=\"" + $last_active_tag.data("id") + "\"]").data({
            ...
            //清空搜索结果
            $("#search_results .by-tag").removeClass("finished").find(".tag-result.show").fadeOut(function() {
                ...
                Note.prototype.check_cache_status(tag_id, last_refresh, num, function(data) {
                    ...
                    if (feedback.status && feedback.status == "ok") {
                        ...
                        if (feedback.cache_status == "invalid" || $("#search_results .by-tag .tag-result.tag-" + tag_id).length == 0) {
                            ...
                            $("#search_results .by-tag .tag-result.tag-" + tag_id).fadeIn(function() {
                                $(this).addClass("show")
                            });
                        }
                    } else {
        ...
        function switch_search() {
            if (!$("#search_area").hasClass("searching-keywords")) {
                ...
                //将搜索框展开，标签搜索栏隐藏
                $("#search_area .by-tag").animate({
                    left: "100%"
                });
                $("#search_area .by-keywords").animate({
                    width: "100%"
                });
        ...
        function get_notes_in_tag(tag_id, limit, offset_id) {
            ...
            //便签容器
            var tag_notes_con;
            tag_notes_con = $("#search_results .by-tag .tag-result.tag-" + tag_id).fadeIn(function() {
                $(this).addClass("show")
            }).get(0);
        ...
        $("#wrapper").on("click", "a.maximize-note", function(event) {
            ...
            $note.find(".note.editable").css({
                "max-height": maxHeight + "px",
                "min-height": minHeight + "px"
            });

            //把其他最大化的关闭
            $maximized_notes.removeClass("maximized").find(".note.editable").css({
                "min-height": 0
            });
        ...
        $("#wrapper").on("click", ".note-con a.pin", function(event) {
            ...
            if ($note.hasClass("maximized")) {
                //减去10像素的margin
                $note.css({
                    "top": ($note.offset().top - 10 - $(window).scrollTop()) + "px"
                }).addClass("fixed");
            }
        });
        ...
        $("#search_results").on("click " + downEvent, "#note_ops .tags a.tag", function(event) {
            ...
            if ($(this).hasClass("choosed")) {
                ...
                if (id && id > 0 && tag_id && tag_id > 0) {
                    var note = new Note({
                        id: id,
                        content: $note.find(content_area).data("value")
                    });
        ...
        function container_onscroll(event) {
            ...
            if ($(target).scrollTop() >= stickyTop + 100) {
                ...
                if (!$("#search_area").hasClass("fixed")) {
                    $("#search_area").addClass("fixed").css({
            ...
            if ($(target).scrollTop() > con_full_height - $(window).height() - 500) {
                ...
                Note.prototype.get_notes_in_tag(tag_id, limit, offset_id, function(data) {
                    ...
                    var tag_notes_con = $("#search_results .by-tag .tag-result.tag-" + tag_id).fadeIn(function() {
                        $(this).addClass("show")
                    }).get(0);
        ...
        $("#wrapper").on("submit", ".note-con form.note", function(event) {
            ...
            if (content.length > 5000) {
                showMessage({
                    type: "warning",
                    msg: "你输入的内容长度(" + content.length + "字符)超过了5000字符，请将超过部分(" + (content.length - 5000) + "字符)保存于另一份便签中"
                });
                return false;
            }
    ...
    function sort_notes() {
        ...
        $("#search_results.results-of-tasks .note-con.task").sortable({
            trigger: "a.drag_trigger",
            itemdata: "id",
            sortdata: "position"
        }, function(sortdata) {
        ...
        if ($(option).hasClass("on")) {
            ...
            if ($(option).hasClass("favicon")) {
                ...
                if (navigator.geolocation) {
                    ...
                    showMessage({
                        type: "error",
                        msg: "配置失败，您的浏览器不支持地理定位，请下载Chrome,Firefox或Safari"
                    });
                } //检查html5特性是否可用结束
            }
        }
            ...
            if ($(this).parent().hasClass("evernote")) {
                ...
                user.cancel_granted("evernote", function(data) {
                    ...
                    if (feedback.status == "ok") {
                        showMessage({
                            type: "success",
                            msg: "你已经取消与Evernote账号同步，下次若需同步需要\"重新授权\""
                        });
                ...
                user.cancel_granted("gtask", function(data) {
                    ...
                    if (feedback.status == "ok") {
                        showMessage({
                            type: "success",
                            msg: "你已经取消与Gtask账号同步，下次若需同步需要\"重新授权\""
                        });
                        $(that).parent().removeClass("on");
                        _gtgranted = false;
                    } else {
                ...
                user.cancel_granted("yinxiang", function(data) {
                    ...
                    if (feedback.status == "ok") {
                        showMessage({
                            type: "success",
                            msg: "你已经取消与印象笔记账号同步，下次若需同步需要\"重新授权\""
                        });
                        $(that).parent().removeClass("on");
                        _yxgranted = false;
                    } else {
